# 5 co. 数字背景（SVG）｜スライド制作 共有

> 数字背景は **SVGで共有・使用**します（PNGは使わない）。HTMLは拡大自在、PowerPoint（2016/365以降）も「挿入→画像→SVG」でベクターのまま使えます。色はアイスブルー `#C3D7EE`（うっすら）。
...
- すべて**透過**。白地でも淡水色地（`#C3D7EE`系）でもそのまま重なる。
- 数字色は `fill="#C3D7EE"`。色を変えたい場合は SVG内の `#C3D7EE` を置換（例：ダーク地用は ice のまま＝CI v2 では白抜きを使わない）。
...
- 配色は3色（白／アイスブルー `#C3D7EE`／リッチブラック `#101820`）のみ。数字は水色、文字は濃紺。
