...
description: プレゼンの内容（テキスト・Markdown・PDF等）を入力すると、SLIDE.mdとSLIDE-PATTERN-*.mdを自動選択・割り当てしてAIツールに渡せる設計書SLIDE-DECK.mdを生成する。「プレゼンの設計書を作って」「スライドデッキを組んで」「SLIDE-DECK.mdを生成して」「このプレゼン内容でスライドを作りたい」「slide-deck-builder」と言われたときに使用する。※5co公式CIスライドは Claude Code＋5co-CI-kit で決定論的に作る（docs/SLIDE-md/SLIDE-md-5co/SLIDE.md 参照）。本スキルは Claude Design／汎用デッキ用の任意経路で、「CIスライド作って」等の5co制作では発火させない。
---
...
カレントディレクトリ内の `docs/SLIDE-md/SLIDE-md-*/SLIDE.md` を検索する。

### 1件だけ見つかった場合

> 「デザインシステムとして `docs/SLIDE-md/SLIDE-md-{source}/SLIDE.md` が見つかりました。これを使用しますか？」
...
>
> 1. `docs/SLIDE-md/SLIDE-md-{source1}` — [SLIDE.mdのOverview「マッチするシーン」の内容]
> 2. `docs/SLIDE-md/SLIDE-md-{source2}` — [同上]
> ...」
...
> 「デザインシステム（SLIDE.md）が `docs/SLIDE-md/` フォルダに見つかりませんでした。`slide-md-creator` スキルでSLIDE.mdを先に生成してから、このスキルを再度呼び出してください。」
...
`docs/SLIDE-PATTERN/SLIDE-PATTERN-INDEX.md` を読み込む。

### 読み込めない場合

> 「`docs/SLIDE-PATTERN/SLIDE-PATTERN-INDEX.md` が見つかりませんでした。`docs/SLIDE-PATTERN/` フォルダにスライドパターンが存在するか確認し、存在する場合は `slide-pattern-creator` スキルを呼び出してインデックスを生成してください。」
...
3. **Slide Patterns** セクションを書く（STEP 5で使用が確定したパターン名をもとに `docs/SLIDE-PATTERN/SLIDE-PATTERN-{name}/SLIDE-PATTERN-{name}.md` を読み込み、全内容をそのままコピーして貼り付ける。使用しないパターンは含めない）
